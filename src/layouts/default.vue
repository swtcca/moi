...
globalState.nav_classes = computed(() => `mt-${Math.floor(heightNavTop.value || 0) + parseInt(top.value.replace(/px/,"") || "0")}px mb-${Math.floor(heightNavBottom.value)}px`)
globalState.nav_top = computed(() => `${Math.floor(heightNavTop.value || 0) + parseInt(top.value.replace(/px/,""))}px`)
globalState.nav_bottom = computed(() => `${Math.floor(heightNavBottom.value || 0)}px`)
globalState.nav_router = computed(() => `h-${heightwindow.value - Math.floor(heightNavBottom.value || 0) - Math.floor(heightNavTop.value || 0) - parseInt(top.value.replace(/px/,"") || "0")}px`)
</script>
