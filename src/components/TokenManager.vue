...
<template>
  <div v-if="user.is" class="py-8 grid grid-cols-1 gap-6 text-gray-700 dark-text-gray-500">
    <h3>Token Manager <div class="i-carbon-save text-2xl text-blue-500" @click="save_tokens"></div><div :class="input_type === 'password' ? 'i-ph-eye-light' : 'i-ph-eye-slash'" class="text-2xl" @click="toggle_type"></div></h3>
    <div v-for="token of Object.keys(tokens)" :key="token">
      <div>
        <label>{{ token }}</label>
        <input placeholder="openai token here, required" class="w-full ring-1" :type="input_type" v-model="tokens[token]" />
      </div>
    </div>
  </div>
</template>
